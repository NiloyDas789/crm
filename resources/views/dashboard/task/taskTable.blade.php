<table id="datatable" class="table table-striped table-bordered dt-responsive nowrap">
    ...
    @foreach ($tasks as $task)
        <tr>
            ...
            <td>{{ $task->title }}</td>
            <td nowrap="nowrap">
                {{-- <a class="btn btn-info" href="{{ route('task.show', $task->id) }}"><i class="fas fa-eye"></i></a> --}}
                {{-- Edit Modal --}}
                <button type="button" class="btn btn-primary btn-md waves-effect waves-light"
                        data-bs-toggle="modal" data-bs-target="#edit{{$task->id}}"><i
                        class="fas fa-edit"></i></button>
                <!-- sample modal content -->
                <div id="edit{{$task->id}}" class="modal fade" tabindex="-1" role="dialog"
                     aria-labelledby="editLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="editLabel">Edit Task</h5>
