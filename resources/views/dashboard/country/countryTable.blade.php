<table id="datatable" class="table table-striped table-bordered dt-responsive nowrap">
    ...
    @foreach ($countries as $country)
        <tr>
            ...
            <td>{{ $country->name }}</td>
            <td nowrap="nowrap">
                {{-- <a class="btn btn-info" href="{{ route('country.show', $country->id) }}"><i class="fas fa-eye"></i></a> --}}
                {{-- Edit Modal --}}
                <button type="button" class="btn btn-primary btn-md waves-effect waves-light"
                        data-bs-toggle="modal" data-bs-target="#edit{{$country->id}}"><i
                        class="fas fa-edit"></i></button>
                <!-- sample modal content -->
                <div id="edit{{$country->id}}" class="modal fade" tabindex="-1" role="dialog"
